        ...
        <AnimatePresence mode="wait">
          {activeTab === 'apps' && (
            <motion.div key="apps" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <h2 className="text-lg font-bold flex items-center gap-2 mb-4 text-pink-700"><UserPlus className="w-5 h-5" /> Новые заявки ({applications.filter(a => a.status === 'pending').length})</h2>
              {applications.filter(a => a.status === 'pending').map(app => (
                <div key={app.id} className="bg-white p-5 rounded-[2rem] shadow-sm border border-pink-50">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleAppAction(app.id, 'approved')} className="flex-1 bg-pink-500 text-white py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-pink-100 active:scale-95 transition-transform"><CheckCircle className="w-4 h-4" /> Одобрить</button>
                    <button onClick={() => handleAppAction(app.id, 'rejected')} className="flex-1 bg-white border border-gray-200 text-gray-500 py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-gray-50 active:scale-95 transition-transform"><XCircle className="w-4 h-4" /> Отказ</button>
                  </div>
                </div>
              ))}
          ...
          {activeTab === 'broadcast' && (
            <motion.div key="broadcast" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <h2 className="text-lg font-bold flex items-center gap-2 mb-4 text-pink-700"><MessageSquare className="w-5 h-5" /> Рассылка</h2>
